  ...
  constructor(options: PanelOptions) {
    ...
    this.riscoComm.on('PanelCommReady', async () => {
      ...
      this.riscoComm.on('NewZoneStatusFromPanel', (data) => {
        logger.log('debug', `[PANEL->APP] ZONE raw: ${data}`);
        const m = data.match(/ZSTT\*?(\d+)/);
        if (!m) {
          logger.log('warn', `[PANEL->APP] ZONE parse failed (no match) for data: ${data}`);
          return;
        }
        const ZId = Number(m[1]);
        if (!Number.isFinite(ZId)) {
          logger.log('warn', `[PANEL->APP] ZONE parse failed (NaN) for data: ${data}`);
          return;
        }
        const status = data.substring(data.indexOf('=') + 1);
        logger.log('debug', `[PANEL->APP] ZONE parsed id=${ZId}, status=${status}`);
        this.zones.byId(ZId).Status = status;
      });
      this.riscoComm.on('NewOutputStatusFromPanel', (data) => {
        logger.log('debug', `[PANEL->APP] OUTPUT raw: ${data}`);
        const OId = parseInt(data.substring(data.indexOf('OSTT') + 4, data.indexOf('=')), 10);
        if (!isNaN(OId)) {
          const status = data.substring(data.indexOf('=') + 1);
          logger.log('debug', `[PANEL->APP] OUTPUT parsed id=${OId}, status=${status}`);
          this.outputs.byId(OId).Status = status;
        } else {
          logger.log('warn', `[PANEL->APP] OUTPUT parse failed for data: ${data}`);
        }
      });
      this.riscoComm.on('NewPartitionStatusFromPanel', (data) => {
        logger.log('debug', `[PANEL->APP] PARTITION raw: ${data}`);
        const PId = parseInt(data.substring(data.indexOf('PSTT') + 4, data.indexOf('=')), 10);
        if (!isNaN(PId)) {
          const status = data.substring(data.indexOf('=') + 1);
          logger.log('debug', `[PANEL->APP] PARTITION parsed id=${PId}, status=${status}`);
          this.partitions.byId(PId).Status = status;
        } else {
          logger.log('warn', `[PANEL->APP] PARTITION parse failed for data: ${data}`);
        }
      });
      this.riscoComm.on('NewMBSystemStatusFromPanel', (data) => {
        logger.log('debug', `[PANEL->APP] SYSTEM raw: ${data}`);
