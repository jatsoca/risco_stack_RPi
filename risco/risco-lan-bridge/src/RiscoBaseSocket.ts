...
export abstract class RiscoBaseSocket extends TypedEmitter<RiscoSocketEvents> {
  ...
  async newDataHandler(data: Buffer) {
    do {
      ...
      logger.log('debug', `[RAW<-PANEL] buffer=${stringedBuffer}`);
      const [receivedId, receivedCommandStr, isCRCOK] = this.rCrypt.decodeMessage(subData);

      logger.log('debug', `[RAW<-PANEL] id=${receivedId} str=${receivedCommandStr} crcOk=${isCRCOK} inCryptTest=${this.inCryptTest}`);
  ...
  private async onCommandBadCrc(receivedId: number | null) {
    this.badCRCCount++;
    logger.log('warn', `Command[${receivedId}] Wrong CRC (count=${this.badCRCCount}/${this.badCRCLimit})`);
